...
Contributors can rename a function; the MCRIT backend records every name as a label by the user who set it, so the *Function Labels* row keeps who named the function when (this needs an MCRIT backend newer than 1.8.1, otherwise the button is disabled):
