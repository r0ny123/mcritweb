...
## Version History
 * unreleased: A cross compare's job page no longer fetches every sub-job and its sample again on each auto-refresh - 81 backend calls every three seconds for 40 samples. It polls the job's own status instead, one call, and reloads once that moves: a sub-job finishing or failing, or the job itself starting, progressing, finishing or failing. A running sub-job's percentage therefore updates at those points rather than on every refresh. Without JavaScript the page refreshes as before (#183).
