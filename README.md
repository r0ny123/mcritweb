 ...
 * unreleased: Query matching results now say which file was queried, and stop giving the queried binary an identity it does not have (#40). The result page showed "Family: *1 | Unnamed", a link to family 0, for a binary that was matched without ever being stored; the Family label and the Version/Component/Library row are now dropped for a sample the corpus does not hold. The filename showed "-" because no MCRIT query endpoint accepts one and the report comes back with an empty name - MCRITweb is the only place that ever sees it, so it now keeps it (BREAKS DB -> CREATE TABLE query_upload); existing deployments gain the table on next start. Also fixes an injection in the copy-to-clipboard button, which built a quoted JavaScript string literal that a sample filename containing a quote could break out of. Merge this line into the next release's entry.
 * unreleased: **Security fix in the query upload path.** `analyze.query` kept a copy of every upload in `instance/temp/uploads/` under a filename taken from a sha256, written with no existence check - and for a `.smda` report that hash was read out of the uploaded report's own `sha256` field, which is a value the uploader types. Any visitor could therefore name their upload after another user's stored query and destroy it, permanently, since nothing restored it. Uploads are now filed under the backend-issued job id, which no request can choose, and are written only once the job has been queued - so a query the backend refuses no longer leaves a file behind either. **This affects existing deployments:** every file already in that folder is hash-named and is not found under the new scheme, so those files become orphans in a folder nothing prunes and can be deleted; correspondingly, a query made before this upgrade cannot be promoted to a sample (#9) - its result page reports the file as no longer available, the same thing it reports for a query submitted through the API. Nothing else reads the folder, and no analysis data is affected. Merge this line into the next release's entry.
 * unreleased: Dark mode, chosen per user under Settings -> Change theme and applied when a page is rendered, so there is nothing to switch and nothing flashes in the wrong palette (#70). (BREAKS DB -> ALTER TABLE user ADD theme VARCHAR; an existing account reads back as `light` and needs no backfill.) `static/style.css` carries both palettes as custom properties and the templates were moved onto them; the vendored Bootstrap is 5.0.2, which predates `data-bs-theme`, so the new `static/theme-dark.css` points its components - and jQuery-UI's, DataTables' and Dropzone's - at the same names from outside, without editing anything under `static/` that we did not write. `ScoreColorProvider` mixes a score tint into the page it is drawn on rather than always into white, which is what stops a dark result table from being a field of near-white blocks; the thresholds and the scores behind them are unchanged. The match diagram PNGs follow the theme too - they are drawn on the ground of the theme that asked for one, and cached under a name that says which, so the two do not overwrite each other in a cache nothing invalidates. Not themed: the CFG panes rendered by the vendored CFGExplorer.
