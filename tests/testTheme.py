...
def test_the_dark_stylesheet_is_linked_after_every_vendored_one(client, as_role):
    ...
    assert body.index("theme-dark.css") > body.index("hint.css")
    # DataTables and jQuery-UI used to be linked here too; #63 took DataTables out
    # entirely and moved jQuery-UI onto the one page that uses it, which loads it from
    # {% block style %} - after this. That one is outranked on specificity instead:
    # every rule in theme-dark.css is scoped to :root[data-theme="dark"].
    assert "dataTables" not in body
    assert "jquery-ui.css" not in body
