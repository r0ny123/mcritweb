...
def test_the_function_comparison_page_shows_the_overall_match_score(function_vs_page, fake_mcrit):
    ...
    Rounded, not truncated: issue #7 replaced this template's `"%d"|format` with
    `"%.0f"|format` along with the rest of the score columns, so 7.8125 reads as 8.
    """
    ...
    assert int(shown.group(1)) == round(float(expected))
