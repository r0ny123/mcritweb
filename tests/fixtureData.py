...
class CorpusMcritClient:
    ...
    def getMatchFunctionVs(self, function_id_a, function_id_b, *args, **kwargs):
        ...
        if minhash_a and minhash_b:
            # float(), because the real client's value has been through JSON - a
            # numpy scalar here would let a view look right against a shape the wire
            # never produces
            score = float(MinHash.calculateMinHashScore(minhash_a, minhash_b, minhash_bits=bits))
        flags = 0
    ...
    def getQueueData(self, start=0, limit=0, method=None, filter=None, state=None, ascending=False):
        ...
        if isinstance(filter, str):
            try:
                ...
                # `Job.parameters` raises for a job whose payload cannot be read, and
                # the expression above is the server's (QueueRemoteCalls.getQueueData),
                # so the whole call fails on the backend. McritClient turns a failed
                # call into None via handle_response - answer that, not the exception,
                # or a view is tested against a failure mode it can never see.
                return None
        return jobs
