...
The clock is `data.job_clock_now` - its own, naive, because a job document's
timestamps are naive, where `db.utc_now` is timezone-aware (issue #98). Every test
that reaches it freezes it, so nothing
...
def frozen_clock(monkeypatch):
    ...
    monkeypatch.setattr(data, "job_clock_now", lambda: NOW)
    return NOW
