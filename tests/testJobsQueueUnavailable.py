...
def test_a_category_the_queue_does_hold_still_paginates(client, as_role, fake_mcrit, monkeypatch):
    ...
    assert response.status_code == 200
    # the corpus fake records its own normalised call, which puts start and limit
    # positionally however the view passed them
    windows = [args[0] if args else kwargs.get("start")
