...
Its own module rather than tests/testBrowser.py, which issue #80 created at the
same time for the unique-blocks clipboard: the two need different `live_server`
and `browser_page` fixtures - this one logs in through the form and collects
console errors, that one signs a session cookie and watches for alert() - and one
file cannot hold both under those names.
