...
def badges(response):
    ...
    for match in JOB_BADGE.finditer(response.get_data(as_text=True)):
        # the flask icon is green when the sample has jobs; issue #70 moved that
        # green out of the markup and into the palette, so the token is what marks it
        if "color:var(--status-yes)" in match.group(0):
...
def test_fetching_the_queue_per_method_still_answers_newest_first(app, corpus_mcrit):
    ...
    # a request context: the collection is built through `describable_jobs`, whose
    # readability memo lives on `g` (issue #51)
...
def test_the_single_sample_page_survives_a_failed_function_search(client, as_role, fake_mcrit):
    ...
    assert response.status_code == 200
    # the wording issue #79 gave this message
