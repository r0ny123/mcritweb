...
- **Do not** modify vendored assets under `static/` (Bootstrap, jQuery, DataTables, Dropzone, Font Awesome, SortableJS, `trace_CFG/`); they carry their own licenses.
  - **`static/trace_CFG/main_duo.js` is not stock either — it is a project fork.** It is 3,839 lines against `main.js`'s 3,533, `diff` between the two is 1,000 lines, and it carries our own comments and patches inline (`// MCRIT resize to minimum of width and height...`, and the CSRF headers marked `// mcritweb: issue #83`). Issue #69 added per-panel graph/loop/node state, the `Show Loops` binding and the per-panel loop boundaries (`renderLoopBoundaries`); it then also made `showGraph`'s three interaction handlers say which panel they are for — the hover no longer falls back to the `#text_code` paragraphs this template has no counterpart for (that fallback threw on every hover, because d3 3.4.11 answers a missed `selectAll` with a selection rather than with nothing), the tooltip writes to this panel's `#tooltip_<id>`/`#value_<id>` instead of the single-graph page's `#tooltip`/`#value`, and the edge handlers are scoped to this panel's container and look a clicked edge up in the graph that was rendered rather than in the global `g` this page never assigns. All of it is marked `// mcritweb: issue #69`. **The tooltip writes its text with `.text()`, and it has to stay that way**: those lines are the dot graph's node label, which carries the api names smda read out of the analysed binary, so the `innerHTML` assignment it replaced ran an import named `<img src=x onerror=...>` — it was unreachable only for as long as the hover threw first. `main.js` still has that same sink, held shut by a throw of its own. Refreshing this file from upstream would silently take the side-by-side page's loop and cycle highlights and its loop boundaries back out, put the throws and the sink back, and re-break the CSRF headers. `loopCollapser.js`, `loopify_dagre.js`, `fnManip.js` and `main.js` itself are untouched and should stay that way — in particular `loopify_dagre` cannot draw the boundaries here: it is a singleton over the globals `dotFile`/`loopsObj`, it hulls a rewritten layout this page does not render, and it appends its one `#bgFill` to a `#graphContainer g.zoom` that exists only on the single-function page. `tests/testFunctionVsBrowser.py` drives what this file draws in Chromium, and what it does with a hover, the tooltip control and an edge click; it needs `playwright`, which is not a dependency, so it skips where that is absent and the markup half stays in `testResultPages.py` for CI.
