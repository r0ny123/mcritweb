...
- **Do not** modify vendored assets under `static/` (Bootstrap, jQuery, DataTables, Dropzone, Font Awesome, SortableJS, `trace_CFG/`); they carry their own licenses.
  - **`static/trace_CFG/main_duo.js` is not stock either — it is a project fork.** It is 3,647 lines against `main.js`'s 3,533, `diff` between the two is 620 lines, and it carries our own comments and patches inline (`// MCRIT resize to minimum of width and height...`, and the CSRF headers marked `// mcritweb: issue #83`). Issue #74 added the two-pane view synchronisation, marked `// mcritweb: issue #74`; the combined view it also asks for is *not* here, and [ADR-0003](docs/adr/0003-no-combined-cfg-view.md) says what it would take. Refreshing this file from upstream would silently take that back out, and re-break the CSRF headers. `loopCollapser.js`, `loopify_dagre.js`, `fnManip.js` and `main.js` itself are untouched and should stay that way.
