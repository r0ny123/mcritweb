...
- **Do not** modify vendored assets under `static/` (Bootstrap, jQuery, DataTables, Dropzone, Font Awesome, SortableJS, `trace_CFG/`); they carry their own licenses.
  - **`static/trace_CFG/main_duo.js` is not stock either — it is a project fork.** It is 3,537 lines against `main.js`'s 3,533, `diff` between the two is 492 lines, and it carries our own comments and patches inline (`// MCRIT resize to minimum of width and height...`, and the CSRF headers marked `// mcritweb: issue #83`). Issue #69 added per-panel graph/loop/node state and the `Show Loops` binding, all marked `// mcritweb: issue #69`, and issue #74 added the two-pane view synchronisation, marked `// mcritweb: issue #74`. Refreshing this file from upstream would silently take the side-by-side page's loop and cycle highlights and that synchronisation back out, and re-break the CSRF headers. `loopCollapser.js`, `loopify_dagre.js`, `fnManip.js` and `main.js` itself are untouched and should stay that way.
