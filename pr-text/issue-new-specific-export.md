...
| `/data/specific_export/family/0` (mcrit's default family, empty here) | 200 | 31.7 MB, all 66 samples |

The family branch has the same hole one step later. Its unknown-id case is a 500 today, but a family that exists and has no samples also reaches `getExportData([])`, and so also exports everything. The likeliest one is family 0, the unnamed family mcrit creates in every storage and keeps even when it has no samples. It is empty whenever every sample has a family, as on this instance, and `/explore/families` lists it with an export button, so one click on that button downloads the whole corpus.
